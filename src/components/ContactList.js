...
const ContactList = () => {
    ...
    const [contacts, setContacts] = useState([
        { id: 1, profile: 'https://via.placeholder.com/40', name: '정웅', nickname: '대학동기', email: 'Jung@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '20학번 동기. 군대 제대함. 여자친구 있음', memo: '시험 기간마다 항상 같이 새벽까지 스터디를 했던 친구.', tone: '따뜻한 말투. 진지하지 않게. 장난스럽게. 사투리 추가.' },
        { id: 2, profile: 'https://via.placeholder.com/40', name: '안예찬', nickname: '대학동기', email: 'Ahn@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '20학번 동기', memo: '친절하고 매사에 성실함', tone: '차분한 말투. 부드럽게.' },
        { id: 3, profile: 'https://via.placeholder.com/40', name: '김소룡', nickname: '대학동기', email: 'Kim@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '20학번 동기', memo: '항상 재밌는 분위기 메이커', tone: '유머러스하게. 가볍게.' },
        { id: 4, profile: 'https://via.placeholder.com/40', name: '김문권', nickname: '대학동기', email: 'Kim@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '친절한 사람', memo: '항상 웃음 짓는 친구', tone: '따뜻하게. 긍정적으로.' },
        { id: 5, profile: 'https://via.placeholder.com/40', name: '임차민', nickname: '대학동기', email: 'Im@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '농담을 잘함', memo: '연락을 자주 하는 친구', tone: '활발하게. 유쾌하게.' },
        { id: 6, profile: 'https://via.placeholder.com/40', name: '윤단비', nickname: '대학동기', email: 'Yun@hansung.ac.kr', phone: '010-1111-2222', group: '찐친', tag: '조용하고 예의 바름', memo: '공부를 열심히 함', tone: '조용하게. 정중하게.' },
        { id: 7, profile: 'https://via.placeholder.com/40', name: '박영수', nickname: '동아리원', email: 'Park@hansung.ac.kr', phone: '010-3333-4444', group: '동아리', tag: '활발한 성격', memo: '다양한 취미를 가진 친구', tone: '자신감 있게. 에너제틱하게.' }
    ]);
    ...
    const filteredContacts = contacts.filter(contact => contact.group === activeTab);
    const 찐친Count = contacts.filter(contact => contact.group === '찐친').length;
    const 동아리Count = contacts.filter(contact => contact.group === '동아리').length;
    ...
    // 선택된 연락처 삭제 함수
    const handleDelete = () => {
        const remainingContacts = contacts.filter(contact => !selectedContacts.includes(contact.id));
        ...
        <div style={styles.container}>
            ...
            <div style={styles.actions}>
                ...
                <div style={styles.buttonsContainer}>
                    ...
                    {/* 연락처 추가 버튼 */}
                    <button style={styles.personalizeButton} onClick={() => navigate('/contact-form')}>
            ...
            {isModalOpen && <PersonalizationModal closeModal={closeModal} />}
                    ...
                    filteredContacts.map((contact) => (
                        <div key={contact.id} style={styles.contactItem}>
                            <div style={styles.contactInfo}>
                                ...
                                />
                                <img src={contact.profile} alt="Profile" style={styles.profileImage} />
                                ...
                                <button onClick={() => toggleDetails(contact.id)} style={styles.detailsButton}>
                                    {expandedContactId === contact.id ? <FaChevronUp /> : <FaChevronDown />}
                                </button>
                            </div>
                                ...
                                <div style={styles.detailsContainer}>
                                    <div style={styles.detailsHeader}>
                                        {isEditing === contact.id ? (
                                            <button style={styles.saveButton} onClick={() => handleSave(contact.id)}>저장</button>
                                        ) : (
                                            <button style={styles.saveButton} onClick={() => handleEdit(contact)}>수정</button>
                                        )}
